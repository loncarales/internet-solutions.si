		...
		if (!is_single()) {
			...
			if($max_page > 1) {
				...
				if ($start_page >= 2 && $pages_to_show < $max_page) {
					...
					echo '<a href="'.clean_url(get_pagenum_link()).'" class="padd-first padd-page-word" title="'.$first_page_text.'">' . Padd_PageNavigation::ANTE_PAGE . $first_page_text . Padd_PageNavigation::POST_PAGE . '</a>';
					echo '<span class="padd-extend">' . Padd_PageNavigation::ANTE_PAGE . '...' . Padd_PageNavigation::POST_PAGE . '</span>';
				}
				if($larger_page_to_show > 0 && $larger_start_page_start > 0 && $larger_start_page_end <= $max_page) {
					for($i = $larger_start_page_start; $i < $larger_start_page_end; $i+=$larger_page_multiple) {
						$page_text = str_replace("%PAGE_NUMBER%", number_format_i18n($i), '%PAGE_NUMBER%');
						echo '<a href="'.clean_url(get_pagenum_link($i)).'" class="padd-page" title="'.$page_text.'">' . Padd_PageNavigation::ANTE_PAGE . $page_text . Padd_PageNavigation::POST_PAGE . '</a>';
					}
				}
				if (($paged - 1) > 0) {
					echo '<a href="'.clean_url(get_pagenum_link($paged - 1)).'" class="padd-page padd-prev" title="Previous">' . Padd_PageNavigation::ANTE_PAGE . '&laquo; Prev' . Padd_PageNavigation::POST_PAGE . '</a>';
				}
				for($i = $start_page; $i  <= $end_page; $i++) {						
					if($i == $paged) {
						$current_page_text = str_replace("%PAGE_NUMBER%", number_format_i18n($i), '%PAGE_NUMBER%');
						echo '<span class="padd-current">'. Padd_PageNavigation::ANTE_PAGE . $current_page_text. Padd_PageNavigation::POST_PAGE . '</span>';
					} else {
						$page_text = str_replace("%PAGE_NUMBER%", number_format_i18n($i), '%PAGE_NUMBER%');
						echo '<a href="'.clean_url(get_pagenum_link($i)).'" class="padd-page" title="'.$page_text.'">' . Padd_PageNavigation::ANTE_PAGE .$page_text . Padd_PageNavigation::POST_PAGE . '</a>';
					}
				...
				if (($paged + 1) <= $max_page) {
					echo '<a href="'.clean_url(get_pagenum_link($paged + 1)).'" class="padd-page padd-next" title="Next">' . Padd_PageNavigation::ANTE_PAGE . 'Next &raquo;' . Padd_PageNavigation::POST_PAGE . '</a>';
				}
				...
				if ($end_page < $max_page) {
					...
					echo '<a href="'.clean_url(get_pagenum_link($max_page)).'" class="padd-last padd-page-word" title="'.$last_page_text.'">' . Padd_PageNavigation::ANTE_PAGE .$last_page_text . Padd_PageNavigation::POST_PAGE . '</a>';
				}
				echo '</div>'.$after."\n";
			}
		}
