...
class PurifiedPlugin extends HP_Plugin
{
	...
	function get_purifier( $for_post = false, $tags = '' ) {
		if ( version_compare( phpversion(), '5.0', '>=' ) )
			include_once dirname( __FILE__ ).'/lib5/HTMLPurifier.auto.php';
		else
	...
	function wp_footer() {
		...
		if ( $options['footer_message'] === true ) {
			...
			if ( $count > 0 )
				$end = sprintf( __ngettext( '%d item has been purified.', '%d items have been purified.', $count, 'html-purified' ), $count );
			...
			echo '<div style="font-size: 10px; border-top: 1px solid #666; padding-left: 2px; background-color: #333; color: #999"><p style="padding: 2px 0px; margin: 0">'.$str.'. '.$end;
		}
	}
}
